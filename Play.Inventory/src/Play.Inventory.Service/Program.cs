    ...
    builder.Services.AddHttpClient<CatalogClient>(client =>
    {
        ...
        .AddTransientHttpErrorPolicy(policyBuilder => policyBuilder.Or<TimeoutRejectedException>().WaitAndRetryAsync(
            ...
            onRetry: (outcome, timespan, retryAttempt) =>
            {
                ...
                serviceProvider.GetService<ILogger<CatalogClient>>()?.LogWarning($"$DelayingFor {timespan.TotalSeconds} seconds, then making retry {retryAttempt} ");
            }
            ))
            ...
            {
                var serviceProvider = builder.Services.BuildServiceProvider();
                serviceProvider.GetService<ILogger<CatalogClient>>()?.LogWarning($"Openning the circuits for {timespan.TotalSeconds} seconds");
            },
            onReset: () =>
            {
                var serviceProvider = builder.Services.BuildServiceProvider();
